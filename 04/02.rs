...
fn main() -> io::Result<()> {
  ...
  let mut old_sum_rolls = sum_rolls;

  // go over the grid over again until nothing changes anymore
  while sum_rolls == 0 || old_sum_rolls != sum_rolls {
    old_sum_rolls = sum_rolls;
    let mut rolls_to_reset: Vec<(usize, usize)> = Vec::new();
    //println!("");
    for y in 0..grid.len() {
      //println!("");
      for x in 0..grid[y].len() {
        ...
        if rolls < 4 && grid[y][x] == 1 {
          sum_rolls += 1;
          rolls_to_reset.push((y, x));
          //println!("change {}", sum_rolls);
          //println!("sum_rolls: {}, grid[{}][{}]={}", sum_rolls, y, x, grid[y][x]);
        }
      }
    }
    // reset the tracked rolls
    for (y, x) in rolls_to_reset {
      grid[y][x] = 0;
    }
  }
