...
fn main() -> io::Result<()> {
  ...
  if let Ok(lines) = read_lines(input) {
    ...
    'outer: while circuits.is_empty() || circuits[0].len() != junctions.len() {
      let item = sorted_distances[limit]; 
      limit = limit + 1;
      println!("{:?}", item);
    ...
    println!("{}", product);
    println!("{:?} {:?}", junctions[sorted_distances[limit-1].0.0], junctions[sorted_distances[limit-1].0.1]);

    // part 2
    println!("{:?} {}", &sorted_distances[..limit], junctions[sorted_distances[limit-1].0.0].x * junctions[sorted_distances[limit-1].0.1].x);
